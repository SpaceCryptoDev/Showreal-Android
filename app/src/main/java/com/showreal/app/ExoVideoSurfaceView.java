...
public class ExoVideoSurfaceView extends GLSurfaceView implements SimpleExoPlayer.VideoListener {
    private static final String TAG = "VideoSurfaceView";
    private VideoRender mRenderer;
    private SimpleExoPlayer mMediaPlayer = null;
    private Context mContext;

    public ExoVideoSurfaceView(Context context) {
        super(context);
        mContext = context;
    ...
    public ExoVideoSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        setEGLContextClientVersion(2);
        mRenderer = new VideoRender(this);
     ...
     *
     * @param mediaPlayer  instance of {@link SimpleExoPlayer}
     * @param shaderEffect any effect that implements {@link ShaderInterface}
     */
    public void init(SimpleExoPlayer mediaPlayer, ShaderInterface shaderEffect) {
        if (mediaPlayer == null) {
        ...
        mRenderer.setEffect(shaderEffect);
    }
    ...
    private void updateVideoSize(float width, float height) {
        mRenderer.viewHeight = getHeight();
        mRenderer.viewWidth = getWidth();
        mRenderer.setVideoSize(width, height);
    }

    public void setVideo(Video video) {
        mRenderer.scaleAdjust = video.scale;
        mRenderer.xAdjust = (float) video.offsetX;
        mRenderer.yAdjust = (float) video.offsetY;
    }
    ...
    private static class VideoRender implements Renderer,
        ...
        private float videoWidth = -1;
        private float videoHeight = -1;
        private float viewWidth = -1;
        private float viewHeight = -1;
        public double scaleAdjust;
        public float yAdjust = -999;
        public float xAdjust = -999;
        public ShaderInterface effect;

        public VideoRender(GLSurfaceView surfaceView) {
...
//            checkGlError("glEnableVertexAttribArray maTextureHandle");
            ...
            if (xAdjust != -999) {
                Matrix.translateM(mMVPMatrix, 0, -xAdjust, -yAdjust, 0);
            }
