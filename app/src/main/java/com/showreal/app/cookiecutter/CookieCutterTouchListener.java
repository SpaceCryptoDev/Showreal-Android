...
public class CookieCutterTouchListener implements View.OnTouchListener {
    ...
    private void onImageDragged(MotionEvent event, ImageView view) {
        ...
        offsetX += (translationX / scaleWidth);
        offsetY += (translationY / scaleHeight);

        Log.i("offsets", String.format("%f, %f", offsetX, offsetY));

        offsetMatrix.postTranslate(translationX * scaleHeight, translationY * scaleHeight);
